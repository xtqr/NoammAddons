...
object Render3D {
    fun renderBlock(
        ...
        val shape =
                if (state.block != Blocks.AIR) state.getShape(mc.level!!, pos) else Shapes.block()
        ...
        if (fill)
                addFilledBoxVertices(
                        ...
                        ctx.consumers.getBuffer(
                                if (phase) NoammRenderLayers.FILLED_THROUGH_WALLS
                                else NoammRenderLayers.FILLED
                        ),
                        x1,
                        y1,
                        z1,
    ...
    fun renderLine(
        ...
        buffer.addVertex(matrix, start.x.toFloat(), start.y.toFloat(), start.z.toFloat())
                .setColor(r, g, b, a)
                .setNormal(matrix, direction)
                .setLineWidth(thickness.toFloat())
        buffer.addVertex(matrix, finish.x.toFloat(), finish.y.toFloat(), finish.z.toFloat())
                .setColor(r, g, b, a)
                .setNormal(matrix, direction)
                .setLineWidth(thickness.toFloat())
    ...
    fun renderTracer(ctx: RenderContext, point: Vec3, color: Color, thickness: Number = 2.5) {
        ...
        ctx.matrixStack.translate(
                -ctx.camera.positionVec.x,
                -ctx.camera.positionVec.y,
                -ctx.camera.positionVec.z
        )

        val buffer =
                (ctx.consumers as MultiBufferSource.BufferSource).getBuffer(
                        NoammRenderLayers.LINES_THROUGH_WALLS
                )
        val cameraPoint =
                ctx.camera.positionVec.add(
                        Vec3.directionFromRotation(ctx.camera.xRot(), ctx.camera.yRot())
                )
        val normal =
                point.toVector3f()
                        .sub(
                                cameraPoint.x.toFloat(),
                                cameraPoint.y.toFloat(),
                                cameraPoint.z.toFloat()
                        )
                        .normalize()
